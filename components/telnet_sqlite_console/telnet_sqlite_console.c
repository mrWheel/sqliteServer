...
static void telnet_reply_nego(int fd, unsigned char cmd, unsigned char opt) {
    ...
    if (opt == TELOPT_AUTH || opt == TELOPT_ENCRYPT) {
        ...
        if (cmd == DO)   { telnet_send_cmd(fd, WONT, opt); return; }
        if (cmd == WILL) { telnet_send_cmd(fd, DONT, opt); return; }
        if (cmd == DONT) { telnet_send_cmd(fd, WONT, opt); return; }
        if (cmd == WONT) { telnet_send_cmd(fd, DONT, opt); return; }
    }
...
static void telnet_drain_negotiation(int fd) {
    ...
    for (int tries = 0; tries < 5; tries++) { // max ~1s totaal
...
static void prompt(int fd) { send_str(fd, "sqlite> "); }
...
static void print_row_sep(int fd) {
    if (s_opt.mode == MODE_TABS) send_str(fd, "\t");
    else send_str(fd, s_opt.sep);
}
...
static void dot_import(int fd, char *args) {
    ...
    for (; i < tokc; i++) {
        ...
        if (strcmp(tokv[i], "--separator") == 0) {
            if (i + 1 >= tokc) { send_str(fd, "Usage: .import --separator X <file> <table>\r\n"); return; }
            sep = tokv[++i][0];
            csv = false;
            continue;
        }
        if (strcmp(tokv[i], "--skip") == 0) {
            if (i + 1 >= tokc) { send_str(fd, "Usage: .import --skip N <file> <table>\r\n"); return; }
            skip = atoi(tokv[++i]);
            if (skip < 0) skip = 0;
            continue;
        }
    ...
    if (!had_error) sqlite_exec_simple(fd, "COMMIT;");
    else sqlite_exec_simple(fd, "ROLLBACK;");
...
static bool handle_dot_command(int fd, char *line) {
    ...
    if (!strcmp(tok, ".help") || !strcmp(tok, ".?")) { dot_help(fd); return true; }
    if (!strcmp(tok, ".quit") || !strcmp(tok, ".exit")) { send_str(fd, "bye\r\n"); shutdown(fd, SHUT_RDWR); return true; }
    if (!strcmp(tok, ".tables")) { dot_tables(fd); return true; }
    if (!strcmp(tok, ".schema")) { dot_schema(fd, (rest && *rest) ? rest : NULL); return true; }

    if (!strcmp(tok, ".headers")) {
        if (!rest || !*rest) { sendf(fd, "headers %s\r\n", s_opt.headers ? "on" : "off"); return true; }
        if (!strcmp(rest, "on")) s_opt.headers = true;
        else if (!strcmp(rest, "off")) s_opt.headers = false;
        else send_str(fd, "Usage: .headers on|off\r\n");
        return true;
    }
    ...
    if (!strcmp(tok, ".mode")) {
        ...
        else if (!strcmp(rest, "csv")) { s_opt.mode = MODE_CSV; strncpy(s_opt.sep, ",", sizeof(s_opt.sep)-1); s_opt.sep[sizeof(s_opt.sep)-1]=0; }
        else if (!strcmp(rest, "tabs")) s_opt.mode = MODE_TABS;
        else send_str(fd, "Usage: .mode list|csv|tabs\r\n");
        return true;
    }

    if (!strcmp(tok, ".separator")) {
        if (!rest || !*rest) { sendf(fd, "separator '%s'\r\n", s_opt.sep); return true; }
    ...
    if (!strcmp(tok, ".timeout")) {
        if (!rest || !*rest) { send_str(fd, "Usage: .timeout <ms>\r\n"); return true; }
        int ms = atoi(rest);
        if (ms < 0) ms = 0;
        xSemaphoreTake(s_db_mutex, portMAX_DELAY);
    ...
    if (!strcmp(tok, ".echo")) {
        if (!rest || !*rest) { sendf(fd, "echo %s\r\n", s_opt.echo ? "on" : "off"); return true; }
        if (!strcmp(rest, "on")) s_opt.echo = true;
        else if (!strcmp(rest, "off")) s_opt.echo = false;
        else send_str(fd, "Usage: .echo on|off\r\n");
        return true;
    }

    if (!strcmp(tok, ".dbinfo")) { dot_dbinfo(fd); return true; }

    if (!strcmp(tok, ".read")) { dot_read(fd, (rest && *rest) ? rest : NULL); return true; }
            ...
            for (int i = 0; i < r; i++) {
                ...
                if (ch == '\r' || ch == '\n') {
                    ...
                    if (line[0] == '.') {
                        ...
                        if (fd != s_client_fd || s_client_fd < 0) goto client_done;
                    } else if (line[0] != 0) {
                        exec_sql_text(fd, line);
                    }

                    llen = 0;
                    if (fd == s_client_fd) prompt(fd);
                    continue;
                }
                // backspace
                if (ch == 0x08 || ch == 0x7F) {
